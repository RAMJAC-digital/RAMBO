...
test "Bus Integration: Write to $0000 visible at all RAM mirrors" {
    ...
    try testing.expectEqual(@as(u8, 0x42), state.busRead(0x0100)); // Base address
    try testing.expectEqual(@as(u8, 0x42), state.busRead(0x0900)); // Mirror 1 (+$0800)
    try testing.expectEqual(@as(u8, 0x42), state.busRead(0x1100)); // Mirror 2 (+$1000)
    try testing.expectEqual(@as(u8, 0x42), state.busRead(0x1900)); // Mirror 3 (+$1800)
}
...
test "Bus Integration: RAM mirroring boundary ($1FFF → $0000)" {
    ...
    try testing.expectEqual(@as(u8, 0xFF), state.busRead(0x07FF));

    // And is visible at all mirror boundaries
    try testing.expectEqual(@as(u8, 0xFF), state.busRead(0x0FFF)); // Mirror 1 boundary
    try testing.expectEqual(@as(u8, 0xFF), state.busRead(0x17FF)); // Mirror 2 boundary
    try testing.expectEqual(@as(u8, 0xFF), state.busRead(0x1FFF)); // Mirror 3 boundary
}
...
test "Bus Integration: Mirroring preserves data across all regions" {
    ...
    state.busWrite(0x0100, 0xBB);
    state.busWrite(0x0200, 0xCC);
    state.busWrite(0x0400, 0xDD);
    state.busWrite(0x07FF, 0xEE);

    // Verify each value is mirrored correctly in all three mirror regions
    // Mirror 1 (+$0800)
    try testing.expectEqual(@as(u8, 0xAA), state.busRead(0x0800));
    try testing.expectEqual(@as(u8, 0xBB), state.busRead(0x0900));
    try testing.expectEqual(@as(u8, 0xCC), state.busRead(0x0A00));
    try testing.expectEqual(@as(u8, 0xDD), state.busRead(0x0C00));
    try testing.expectEqual(@as(u8, 0xEE), state.busRead(0x0FFF));

    // Mirror 2 (+$1000)
    try testing.expectEqual(@as(u8, 0xAA), state.busRead(0x1000));
    try testing.expectEqual(@as(u8, 0xBB), state.busRead(0x1100));
    try testing.expectEqual(@as(u8, 0xCC), state.busRead(0x1200));
    try testing.expectEqual(@as(u8, 0xDD), state.busRead(0x1400));
    try testing.expectEqual(@as(u8, 0xEE), state.busRead(0x17FF));

    // Mirror 3 (+$1800)
    try testing.expectEqual(@as(u8, 0xAA), state.busRead(0x1800));
    try testing.expectEqual(@as(u8, 0xBB), state.busRead(0x1900));
    try testing.expectEqual(@as(u8, 0xCC), state.busRead(0x1A00));
    try testing.expectEqual(@as(u8, 0xDD), state.busRead(0x1C00));
    try testing.expectEqual(@as(u8, 0xEE), state.busRead(0x1FFF));
}
...
test "Bus Integration: Write to mirror affects base and all other mirrors" {
    ...
    try testing.expectEqual(@as(u8, 0x88), state.busRead(0x0234));

    // And all other mirrors
    try testing.expectEqual(@as(u8, 0x88), state.busRead(0x0A34)); // Mirror 1 ($0800 + $0234)
    try testing.expectEqual(@as(u8, 0x88), state.busRead(0x1234)); // Mirror 2 (where we wrote)
    try testing.expectEqual(@as(u8, 0x88), state.busRead(0x1A34)); // Mirror 3 ($1800 + $0234)
}
...
test "Bus Integration: PPU registers mirrored every 8 bytes" {
    ...
    const ctrl_value = state.busRead(0x2000);
    try testing.expectEqual(ctrl_value, state.busRead(0x2008)); // First mirror
    try testing.expectEqual(ctrl_value, state.busRead(0x2010)); // Second mirror
    try testing.expectEqual(ctrl_value, state.busRead(0x3000)); // Far mirror
    try testing.expectEqual(ctrl_value, state.busRead(0x3FF8)); // Last mirror before boundary
}
...
test "Bus Integration: PPU mirroring boundary ($3FFF → $2007)" {
    ...
    state.busWrite(0x2006, 0x00); // Low byte
    ...
    _ = state.busRead(0x3FFF); // Dummy read (buffering)
    const value = state.busRead(0x3FFF); // Actual data
...
test "Bus Integration: All PPU register mirrors route to same underlying register" {
    ...
    state.busWrite(0x2006, 0x20); // High byte
    state.busWrite(0x2806, 0x00); // Low byte via mirror (+$0800)

    // The address should now be $2000
    // Write data via another mirror
    state.busWrite(0x3007, 0xAB); // PPUDATA via mirror

    // Verify the PPU VRAM received the write (address $2000 maps to VRAM index 0)
    try testing.expectEqual(@as(u8, 0xAB), state.ppu.vram[0]);
}
...
test "Bus Integration: ROM write does not modify cartridge" {
    ...
    state.busWrite(0x8000, 0x11);
    state.busWrite(0x8064, 0x22);
...
test "Bus Integration: Read from unmapped address returns last bus value" {
    ...
    const unmapped = state.busRead(0x4018);
...
test "Bus Integration: PPU status bits 0-4 are open bus" {
    ...
    state.busWrite(0x2001, 0b00011111); // Write to PPUMASK
...
test "Bus Integration: $8000-$FFFF address range (without cartridge)" {
    ...
    const rom_read = state.busRead(0x8000);
    try testing.expectEqual(@as(u8, 0xAA), rom_read); // Returns open bus

    // Different ROM addresses should all return open bus
    try testing.expectEqual(@as(u8, 0xAA), state.busRead(0xC000));
    try testing.expectEqual(@as(u8, 0xAA), state.busRead(0xFFFF));
}
...
test "Bus Integration: Multiple components share same bus" {
    ...
    // 1. Write to RAM
    state.busWrite(0x0100, 0x11);
    try testing.expectEqual(@as(u8, 0x11), state.busRead(0x0100));

    // 2. Write to PPU register
    state.busWrite(0x2000, 0x80);

    // 3. Read from ROM (without cartridge, returns open bus)
    state.busWrite(0x0200, 0xEE); // Set open bus
    try testing.expectEqual(@as(u8, 0xEE), state.busRead(0x8000));

    // All should work without interference
    try testing.expectEqual(@as(u8, 0x11), state.busRead(0x0100)); // RAM still intact
}
...
test "Bus Integration: read16 works across bus boundaries" {
    ...
    state.busWrite(0x0101, 0x12); // High byte

    const value = state.busRead16(0x0100);
    try testing.expectEqual(@as(u16, 0x1234), value); // Little-endian

    // Test read16 at RAM mirror boundary
    state.busWrite(0x07FF, 0xCD); // Last byte of RAM
    state.busWrite(0x0800, 0xAB); // First byte of mirror (wraps to $0000)

    const boundary_value = state.busRead16(0x07FF);
