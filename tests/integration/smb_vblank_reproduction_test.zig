...
test "SMB VBlank Regression: Super Mario Bros initializes rendering" {
    ...
    while (frame_count < 180) {
        ...
        if (state.vblank_ledger.last_set_cycle > cycle_before_frame) {
            vblank_set_count += 1;
            last_set_cycle = state.vblank_ledger.last_set_cycle;
            last_clear_cycle = state.vblank_ledger.last_clear_cycle;
