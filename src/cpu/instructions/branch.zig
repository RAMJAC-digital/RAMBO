...
/// Branch if C = 0
pub fn bcc(state: *State, bus: *Bus) bool {
    if (state.p.carry) {
...
/// Branch if C = 1
pub fn bcs(state: *State, bus: *Bus) bool {
    if (!state.p.carry) {
...
/// BEQ - Branch if Equal
/// Branch if Z = 1
pub fn beq(state: *State, bus: *Bus) bool {
    if (!state.p.zero) {
        return true; // Not taken
    }
    return performBranch(state, bus);
...
/// Branch if Z = 0
pub fn bne(state: *State, bus: *Bus) bool {
    if (state.p.zero) {
        return true; // Not taken
    }
    return performBranch(state, bus);
...
/// BMI - Branch if Minus
/// Branch if N = 1
pub fn bmi(state: *State, bus: *Bus) bool {
    if (!state.p.negative) {
        return true; // Not taken
    }
    return performBranch(state, bus);
...
/// BPL - Branch if Plus
/// Branch if N = 0
pub fn bpl(state: *State, bus: *Bus) bool {
    if (state.p.negative) {
        return true; // Not taken
    }
    return performBranch(state, bus);
...
/// Branch if V = 0
pub fn bvc(state: *State, bus: *Bus) bool {
    if (state.p.overflow) {
        return true; // Not taken
    }
    return performBranch(state, bus);
...
/// Branch if V = 1
pub fn bvs(state: *State, bus: *Bus) bool {
    if (!state.p.overflow) {
        return true; // Not taken
    }
    return performBranch(state, bus);
...
fn performBranch(state: *State, bus: *Bus) bool {
